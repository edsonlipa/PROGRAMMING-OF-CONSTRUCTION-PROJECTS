...
void cruzamiento(int ind1,int ind2) {
  ...
  int inicio=n_g/3;
  int fin=n_g*2/3;
  for (size_t i = inicio; i <= fin; i++) {//se intercambia los 3 ultimos bits
...
void seleccion_ruleta(){
  // std::cout << "seleccion por ruleta" << '\n';
  std::vector<float> p100(n_p);
  int ind1=0;
  int ind2=0;
  float total=0;
  for (size_t i = 0; i < n_p; i++) {
      total+=abs(poblacion[i].flujo_de_caja);
  }
    //std::cout << "total: " <<total<< '\n';
   for (size_t i = 0; i < n_p; i++) {
      if (i==0) {
      p100[i]=(100*abs(poblacion[i].flujo_de_caja)/total);

      }else{
      p100[i]=((100*abs(poblacion[i].flujo_de_caja)/total)+p100[i-1]);

      }
      // std::cout << i<<") -- " << p100[i]<<'\n';
    }
    float temp_rand1=rand()%100;
    float temp_rand2=rand()%100;
        //cout<<temp_rand1<<" "<<temp_rand2<<endl;
    for(int i=0;i<n_p;i++){
      if(temp_rand1>p100[i] ) ind1=i;
    }
    for(int i=0;i<n_p;i++){
      if(temp_rand2>p100[i]) ind2=i;
    }
    while( ind1==ind2){
      // cout<<temp_rand1<<" $ "<<temp_rand2<<endl;
      temp_rand2=rand()%100;
   ...
   for (size_t t = 0; t < n_i; t++) {
    // std::cout << "\tGeneracion "<<t+1 << '\n';
     for (size_t i = 0; i < n_p; i++) {
       ...
       if(t==0){mostrar_periodos(poblacion[i].programacion);std::cout << "\n/* fitness "<<poblacion[i].flujo_de_caja << '\n';}
     }
